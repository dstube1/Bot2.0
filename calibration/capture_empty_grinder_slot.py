...
def get_grinder_region():
    ...
    if not entry:
        print("'first_slot_grinder' entry missing in scan_windows.json")
        return None
    reg = entry.get('window') or entry.get('region')
    if not (isinstance(reg, (list, tuple)) and len(reg) == 4):
        print("invalid region format for first_slot_grinder")
        return None
